var JSMpeg = { Player: null, VideoElement: null, BitBuffer: null, Source: {}, Demuxer: {}, Decoder: {}, Renderer: {}, AudioOutput: {}, Now: function() { return self.performance ? self.performance.now() / 1e3 : Date.now() / 1e3 }, CreateVideoElements: function() { var elements = self.querySelectorAll(".jsmpeg"); for (var i = 0; i < elements.length; i++) { new JSMpeg.VideoElement(elements[i]) } }, Fill: function(array, value) { if (array.fill) { array.fill(value) } else { for (var i = 0; i < array.length; i++) { array[i] = value } } }, Base64ToArrayBuffer: function(base64) { var binary = self.atob(base64); var length = binary.length; var bytes = new Uint8Array(length); for (var i = 0; i < length; i++) { bytes[i] = binary.charCodeAt(i) } return bytes.buffer }, WASM_BINARY_INLINED: null };
if (self.readyState === "complete") { JSMpeg.CreateVideoElements() } else { self.addEventListener("DOMContentLoaded", JSMpeg.CreateVideoElements) } JSMpeg.VideoElement = function() {
    "use strict";
    var VideoElement = function(element) {
        ...
        this.canvas = self.createElement("canvas");
        this.canvas.width = 960;
        this.canvas.height = 540;
        addStyles(this.canvas, { display: "block", width: "100%" });
        this.container.appendChild(this.canvas);
        this.playButton = self.createElement("div");
        ...
        if (this.player.audioOut && !this.player.audioOut.unlocked) {
            ...
            if (options.autoplay || this.player.options.streaming) {
                this.unmuteButton = self.createElement("div");
...
JSMpeg.Player = function() {
    ...
    var Player = function(url, options) {
        ...
        if (options.pauseWhenHidden !== false) { self.addEventListener("visibilitychange", this.showHide.bind(this)) }
        if (this.wasmModule) {
...
JSMpeg.Decoder.MP2Audio = function() {
    ...
    var MP2 = function(options) {
        ...
        this.D.set(MP2.SYNTHESIS_self, 0);
        this.D.set(MP2.SYNTHESIS_self, 512);
            ...
            for (var granule = 0; granule < 4; granule++) {
                ...
                for (var p = 0; p < 3; p++) {
                    ...
                    for (var ch = 0; ch < 2; ch++) {
                        ...
                        while (vIndex < 1024) {
                            for (var i = 0; i < 32; ++i) { this.U[i] += this.D[dIndex++] * this.V[ch][vIndex++] } vIndex += 128 - 32;
                            dIndex += 64 - 32
                        }
...
JSMpeg.Renderer.WebGL = function() {
    ...
    var WebGLRenderer = function(options) {
        this.canvas = options.canvas || self.createElement("canvas");
    ...
    WebGLRenderer.IsSupported = function() { try { if (!self.WebGLRenderingContext) { return false } var canvas = self.createElement("canvas"); return !!(canvas.getContext("webgl") || canvas.getContext("experimental-webgl")) } catch (err) { return false } };
...
JSMpeg.AudioOutput.WebAudio = function() {
    ...
    var WebAudioOut = function(options) {
        this.context = WebAudioOut.CachedContext = WebAudioOut.CachedContext || new(self.AudioContext || self.webkitAudioContext);
    ...
    WebAudioOut.IsSupported = function() { return self.AudioContext || self.webkitAudioContext };
    WebAudioOut.CachedContext = null;
    return WebAudioOut
}();
JSMpeg.WASMModule = function() {
    ...
    WASM.IsSupported = function() { return !!self.WebAssembly };
